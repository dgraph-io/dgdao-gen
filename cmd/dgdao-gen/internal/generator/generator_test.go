...
func TestGenerateHeader(t *testing.T) {
	...
	for _, entry := range entries {
		...
		t.Run(entry.Name(), func(t *testing.T) {
			...
			if !strings.HasPrefix(string(data), "// Code generated by dgdao-gen. DO NOT EDIT.") {
...
// TestGenerate_LicenseHeader asserts that a license header set on Config is
// rendered as a valid Go comment above the "Code generated" marker, for both
// an already-commented block and a plain-text line.
func TestGenerate_LicenseHeader(t *testing.T) {
	dir := moviesDir(t)
	pkg, err := parser.Parse(dir)
	if err != nil {
		t.Fatalf("Parse(%s) failed: %v", dir, err)
	...
	tests := []struct {
		...
		{
			name:   "AlreadyCommentedBlock",
			header: "/*\n * Copyright 2026 Example Corp.\n * SPDX-License-Identifier: Apache-2.0\n */",
		},
		{
			name:   "PlainTextLine",
			header: "Copyright 2026 Example Corp. All rights reserved.",
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			cfg.LicenseHeader = tt.header
			if err := Generate(pkg, cfg); err != nil {
				t.Fatalf("Generate: %v", err)
			}

			data := mustReadGen(t, tmpDir, "marker_gen.go")

			wantComment := commentHeader(tt.header)
			if !strings.HasPrefix(data, wantComment) {
				t.Fatalf("generated file does not begin with the rendered license header %q:\n%s", wantComment, data)
			}

			rest := strings.TrimPrefix(data, wantComment)
			rest = strings.TrimLeft(rest, "\n")
			if !strings.HasPrefix(rest, "// Code generated by dgdao-gen. DO NOT EDIT.") {
				t.Fatalf("generated-code marker does not immediately follow the license header:\n%s", data)
			}

			fset := token.NewFileSet()
			if _, err := goparser.ParseFile(fset, "marker_gen.go", data, goparser.PackageClauseOnly); err != nil {
				t.Fatalf("output is not valid Go after prepending the license header: %v\n---file---\n%s", err, data)
			}
		})
	}
}

// TestGenerate_NoLicenseHeaderUnchanged asserts that leaving Config.LicenseHeader
// empty (the default, no flag set) leaves generated output unchanged: the
// "Code generated" marker remains the first line.
func TestGenerate_NoLicenseHeaderUnchanged(t *testing.T) {
	dir := moviesDir(t)
	pkg, err := parser.Parse(dir)
	if err != nil {
		t.Fatalf("Parse(%s) failed: %v", dir, err)
	...
	tmpDir := t.TempDir()
	cfg := flatConfig(pkg, tmpDir)
	// cfg.LicenseHeader intentionally left empty.
	if err := Generate(pkg, cfg); err != nil {
		t.Fatalf("Generate: %v", err)
	}

	data := mustReadGen(t, tmpDir, "marker_gen.go")
	if !strings.HasPrefix(data, "// Code generated by dgdao-gen. DO NOT EDIT.") {
		t.Fatalf("marker should still be the first line when no license header is set:\n%s", data)
	}
}

// TestCommentHeader exercises the commentHeader helper directly: already-
// commented input passes through unchanged (trailing newlines trimmed), plain
// text gets "// "-prefixed per line, and empty/whitespace-only input yields "".
func TestCommentHeader(t *testing.T) {
	tests := []struct {
		...
		{name: "LineComment", in: "// already a comment\n", want: "// already a comment"},
		{
			name: "BlockComment",
			in:   "/*\n * block\n */\n",
			want: "/*\n * block\n */",
		...
		{
			name: "PlainSingleLine",
			in:   "Copyright 2026 Example Corp.",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := commentHeader(tt.in); got != tt.want {
				t.Errorf("commentHeader(%q) = %q, want %q", tt.in, got, tt.want)
			}
		})
	}
}
...
`
	...
	for _, want := range []string{
		...
		`func (c *Client) DgdaoClient() dgdao.Client`,
		`return c.conn`,
		`func (c *Client) DropAll(ctx context.Context) error`,
		`func (c *Client) AlterSchema(ctx context.Context, schema string) error`,
