...
const header = "// Code generated by dgdao-gen. DO NOT EDIT.\n\n"

// licenseHeader, when non-empty, is rendered as a Go comment and prepended to
// every generated file, above header. It is set once per Generate call from
// Config.LicenseHeader and read by writeGoFile; writeGoFile is a free function
// called from many sites, so threading it through as a parameter everywhere
// would be needlessly invasive.
...
type Config struct {
	...
	WithValidator bool   // wire in dgdao.WithValidator in the generated CLI
	...
	// LicenseHeader, when non-empty, is rendered as a Go comment (via
	// commentHeader) and prepended above the "Code generated" marker in every
	// generated file. Text already in comment form (starting with "//" or
	// "/*") is passed through unchanged; plain text gets "// " prefixed onto
	// each line.
...
func Generate(pkg *model.Package, cfg Config) error {
	// Render the license header (if any) once per run; writeGoFile reads it
	// from the package-level var for every file it writes.
	licenseHeader = commentHeader(cfg.LicenseHeader)
...
// writeGoFile prepends the license header (if set) and the generated-code
// header, gofmt-formats the result, and writes it to path. On a formatting
// error the raw output is written to path+".broken" for debugging.
...
// commentHeader returns raw as valid Go comment lines. Input already in
// comment form (a line starting with "//" or a block starting with "/*")
// is returned unchanged; otherwise each line is prefixed with "// ".
func commentHeader(raw string) string {
...
// gofmt-style output: standard library, the dgdao module, then the schema
// package plus any external (enum/scalar) packages.
type genImports struct {
	std   map[string]bool       // stdlib import paths
	mg    map[string]bool       // dgdao module import paths
...
func (g *genImports) addClientSideImports(schemaPath string, entity model.Entity) {
	...
	if len(filterableScalarFields(entity.Fields)) > 0 {
		g.mg["github.com/dgraph-io/dgdao/typed/filter"] = true
	}
	g.other[schemaPath] = ImportSpec{Path: schemaPath}
}

// block renders the accumulated imports as a gofmt-style import block, with a
// blank line between the stdlib, dgdao, and schema/external groups. It
